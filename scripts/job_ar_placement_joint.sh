...
# STAGE-3 RECOVERY job for the JOINT generation-LM chain (job_ar_chain_joint.sh).
...
#
# This job runs ONLY stage 3 (placement, goodmix_joint ~286M tok x4ep, ~1-2h) and
# produces lm_placement_joint2, the checkpoint ctbench/variants.py expects for the
# joint_nocasf generation arm. Submit with -hold_jid <chain job id> so it starts
# the moment the chain job ends (by qdel or walltime).
#
...
echo "AR JOINT PLACEMENT DONE (-> lm_placement_joint2)"
