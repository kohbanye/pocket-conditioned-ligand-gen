...
BIO2TOKEN_PYTHON = os.environ.get(
    "RECON_BENCH_BIO2TOKEN_PYTHON",
    str(_BIO2TOKEN_VENV_PY) if _BIO2TOKEN_VENV_PY.exists() else "python",
)

WEIGHTS_DIR = Path(os.environ.get("RECON_BENCH_WEIGHTS_DIR", REPO_ROOT / "weights"))
DATA_DIR = Path(os.environ.get("RECON_BENCH_DATA_DIR", REPO_ROOT / "data"))
OUTPUTS_DIR = Path(os.environ.get("RECON_BENCH_OUTPUTS_DIR", REPO_ROOT / "outputs"))
RESULTS_DIR = Path(os.environ.get("RECON_BENCH_RESULTS_DIR", REPO_ROOT / "results"))
...
OWN_MODEL_WORKDIR = Path(
    os.environ.get("RECON_BENCH_OWN_MODEL_WORKDIR", MONOREPO_ROOT)
)
OWN_MODEL_PYTHON = os.environ.get(
    "RECON_BENCH_OWN_MODEL_PYTHON", str(OWN_MODEL_WORKDIR / ".venv" / "bin" / "python")
)
...
OWN_VQ_RUNS_DIR = Path(
    os.environ.get("RECON_BENCH_OWN_VQ_RUNS_DIR", OWN_MODEL_WORKDIR / "pocket-ligand-vqvae")
)
OWN_ALLATOM_CACHE = Path(
    os.environ.get(
        "RECON_BENCH_OWN_ALLATOM_CACHE",
...
OWN_LOCALFRAME_CACHE = Path(
    os.environ.get(
        "RECON_BENCH_OWN_LOCALFRAME_CACHE",
...
RECEPTOR_CACHE = Path(
    os.environ.get("RECON_BENCH_RECEPTOR_CACHE", str(OUTPUTS_DIR / "receptor_cache"))
)
...
OBABEL = os.environ.get("RECON_BENCH_OBABEL") or shutil.which("obabel") or "obabel"
