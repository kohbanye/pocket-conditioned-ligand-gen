...
def load_rescorer(
    ...
    The checkpoint's own config is the base, because several fields decide what
    submodules exist: ``atom_aux_weight > 0`` adds ``atom_head``, and
    ``head_interaction_layers`` adds encoder blocks. Rebuilding the config from
    the caller's arguments alone drops those, and ``load_state_dict`` then fails
    on the extra keys -- which is exactly what an atom-aux head did here. The
    caller's arguments are applied on top so old checkpoints, which predate the
    config being pickled, still load.
    """
    ...
    if cfg is None:
        ...
        if interaction_layers:
            cfg.head_interaction_layers = interaction_layers
