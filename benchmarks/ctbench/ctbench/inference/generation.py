...
if TYPE_CHECKING:
    ...
    from ctbench.config import GenerationConfig, PathsConfig
    from ctbench.variants import GenerationCkpts
...
def evaluate_with_sbdd(
    ...
    :mod:`ctbench.baselines.sbdd_gen` then collects into this repo's dumps.
    """
...
# contract (see sbddbench/adapters/own.py) — no per-variant code in sbdd-bench.
# ---------------------------------------------------------------------------
...
def evaluate_own_crossdocked(  # noqa: PLR0913
    ...
    *,
    ...
    the exact layout ``ctbench.report`` reads for ``results/generation/<variant>``.
    """
