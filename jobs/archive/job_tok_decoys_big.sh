...
#   .venv/bin/python pipelines/corpora/concat_decoy_shards.py data/lm_tokens_decoys_big
...
.venv/bin/python pipelines/corpora/tokenize_decoys.py \
    ...
    --min-heavy 35 --max-heavy 200 --n-complexes 60000 --n-decoys 16 --n-conformer-decoys 8 \
    --num-shards "$NSHARD" --shard-id $((SGE_TASK_ID - 1)) \
    --out-dir "data/lm_tokens_decoys_big/shard$((SGE_TASK_ID - 1))"
